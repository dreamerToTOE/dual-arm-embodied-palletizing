# Task25-A：分批到料的 Isaac ROS 2 物理桥（侧吸 + 休眠区瞬移）。
#
# 仅在 Task25 场景 Timeline Play 后运行。Bridge 不选择任务、不规划轨迹，只负责：
#   1. 真实侧面 Surface Gripper；
#   2. 八件 Cube 的 Ground Truth PoseArray；
#   3. 原子双臂关节下发（共同命令必须同 stamp 成对写入）；
#   4. 分批到料：按 /task25/feed_command 把对应批次从休眠位瞬移到供料槽，
#      落稳后通过 /task25/feed_state 报告“已到位且静止”。
#
...
# 共同负载阶段由同一个 physics callback 原子写入左右目标。Task25 使用独立
# frame_id，避免与 Task24 执行器的消息混入同一对（两个执行器不得同时运行）。
DUAL_SYNC_FRAME_ID = "task25_dual_sync"
...
# 到料下落高度由场景标记 task25_feed_settle_offset_z 提供，不在这里重复硬编码。
...
def _task_custom(key):
    ...
    if not task_root.IsValid():
        raise RuntimeError(f"缺少 {TASK_ROOT}。请先运行 Task25 场景脚本。")
    value = task_root.GetCustomDataByKey(key)
    if value is None:
        raise RuntimeError(f"Task25 场景缺少标记 {key}。")
    return value
...
class Task25BatchedFeedBridge:
    def __init__(self):
        self.stage = _stage()
        active_count = int(_task_custom("task25_active_cube_count"))
        self.batch_size = int(_task_custom("task25_batch_size"))
        self.batch_count = int(_task_custom("task25_batch_count"))
        if active_count != self.batch_size * self.batch_count:
            raise RuntimeError(
                "Task25 场景的件数与批次定义不一致："
                f"{active_count} != {self.batch_size} x {self.batch_count}。"
            )
        self.slot_poses = {
            "A": _json_tuple(_task_custom("task25_slot_a")),
            "B": _json_tuple(_task_custom("task25_slot_b")),
        }
        ...
        if missing:
            raise RuntimeError("缺少 Task25 Supply Cube：" + ", ".join(missing))

        metadata = []
        for path in self.cube_paths:
            raw = self.stage.GetPrimAtPath(path).GetCustomDataByKey("task25_metadata")
            if raw is None:
                raise RuntimeError(f"{path} 缺少 task25_metadata 标记。")
        ...
        self.feed_settle_offset_z = float(_task_custom("task25_feed_settle_offset_z"))
        ...
        for index, path in enumerate(self.cube_paths, start=1):
            rigid = SingleRigidPrim(path, name=f"task25_cube_{index:02d}")
            rigid.initialize()
        ...
        self.node = rclpy.create_node("task25_batched_feed_bridge", context=self.context)
        ...
        for side in SIDES:
            self.command_subs[side] = self.node.create_subscription(
                Bool, f"/task25/{side}/suction_command",
            ...
            self.state_pubs[side] = self.node.create_publisher(
                Bool, f"/task25/{side}/suction_state", 10
            )
            self.tcp_pubs[side] = self.node.create_publisher(
                PoseStamped, f"/task25/{side}/side_suction_tcp_pose", 10
            )
        self.cube_pub = self.node.create_publisher(PoseArray, "/task25/cube_poses", 10)
        self.feed_state_pub = self.node.create_publisher(
            Int32MultiArray, "/task25/feed_state", 10
        )
        self.feed_command_sub = self.node.create_subscription(
            Int32, "/task25/feed_command", self._feed_command, 10
        )
        ...
        print("\n====================================================")
        print("Task25 batched-feed side-suction ROS bridge started")
        ...
        print("PUB: /task25/{left,right}/suction_state std_msgs/Bool")
        print("PUB: /task25/{left,right}/side_suction_tcp_pose geometry_msgs/PoseStamped")
        print(
    ...
    def _build_gripper(self, side):
        # 与 Task25 MoveIt URDF 一致：L 型工具挂在 fr3_link8。
        ...
        for name in (
            ...
            "task25_side_surface_gripper_joint",
        ):
        ...
        props.d6JointPath = f"{mount_path}/task25_side_surface_gripper_joint"
    ...
    def _activate_batch(self, batch, automatic=False):
        with self._lock:
            if not 1 <= batch <= self.batch_count:
                print(f"[Task25 Isaac] feed_command 批次越界：{batch}")
                return False
            if batch in self.released_batches:
                print(f"[Task25 Isaac] 批 {batch} 已释放过，忽略重复命令。")
                return False
            earlier = [
            ...
            if not_arrived:
                print(
                    f"[Task25 Isaac] 拒绝释放批 {batch}：更早的 Cube {not_arrived} 尚未"
        ...
        for index in indices:
            ...
            print(
                "[Task25 Isaac] 批 %d%s：Cube_%02d -> 槽 %s %s，"
                "底面高于桌面 %.1f mm，等待落稳" %
                (
                    batch,
    ...
    def _check_arrival(self, index, dt):
        ...
        if self.stable_accum[index] >= ARRIVAL_HOLD_SEC:
            ...
            print(
                "[Task25 Isaac] Cube_%02d 到位且静止：pos=(%.3f, %.3f, %.3f)，"
                "|v|=%.4f m/s，|w|=%.4f rad/s" %
                (
                    index + 1,
    ...
    def _joint_command(self, side, message):
        if not message.name or len(message.name) != len(message.position):
            print(f"[Task25 Isaac][{side}] ignored malformed joint command")
            return
        command = (
            ...
            if desired[side]:
                ...
                if not gripper.is_closed():
                    ...
                    if self._retry_accum[side] >= self._retry_interval:
                        self._retry_accum[side] = 0.0
                        if gripper.close():
                            print(f"[Task25 Isaac][{side}] side suction retry -> CLOSED")
            else:
                self._retry_accum[side] = 0.0
...
builtins._task25_batched_feed_bridge = Task25BatchedFeedBridge()
