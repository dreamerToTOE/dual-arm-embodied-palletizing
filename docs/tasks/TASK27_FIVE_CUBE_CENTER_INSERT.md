...
状态：✅ **2026-09-26：当前 `1.5 mm` 内侧余量版本在 Isaac Sim 4.5 + MoveIt2 中单进程连续完成五件实体码垛，进程 exit 0，双臂返回共同 HOME。** 2026-09-24 的分阶段五件通过记录保留如下；两次采用不同几何参数，尚不能证明当前版本的统计稳定性。
...
- 因而五件之间不可消除的总余量为 `28 mm`；
- 最新内侧直推方案在两侧外件与内件之间各预留 `1.5 mm` 名义角点扫掠余量，
  若外侧件贴墙、中心件位于 `y=0`，中央两侧名义间隙各约 `12.5 mm`。
...
|---|---:|---|
| `+2` | `+0.254` | `+Y` 围墙 |
| `+1` | `+0.1325` | 与 `+2` Cube 名义间隙 `1.5 mm` |
| `0` | `0.000` | 左右均保留约 `12.5 mm` 插入余量 |
| `-1` | `-0.1325` | 与 `-2` Cube 名义间隙 `1.5 mm` |
| `-2` | `-0.254` | `-Y` 围墙 |
...
3. 左右间隙总和接近分配内侧通道余量后的名义值 `25 mm`，且中心偏置受限；
4. 两侧间隙尽量均衡。历史零余量版本的实测为 `12.314 mm` / `14.151 mm`，中心偏置约 `0.919 mm`；当前 `1.5 mm` 内侧余量版本的理论中心两侧为 `12.5 mm` / `12.5 mm`；
5. 完成后双臂回共同 HOME。
...
1. 在当前 `1.5 mm` 几何与相同控制参数下，从全新 Isaac 场景至少再跑数轮完整五件，记录每轮是否 exit 0、各件周期、格位误差和分段推入扭矩；不要把不同余量版本合并为同配置成功率。
