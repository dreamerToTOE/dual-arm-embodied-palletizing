...
| 09 | 松协调局部重规划 | 安全退出 `RETREAT -> HOME` + 冲突窗口局部等待，并以共享时钟完成双 FR3 的 Isaac 抓取、放置和安全退出 | ✅ 已完成 |
| 10 | 松协调完整码垛 Demo | 四箱、两批次连续 Isaac 码垛；第二批保留第一批实际放置箱体为碰撞物，全部经 FCL 验收 | ✅ 已完成 |
| 11 | 双吸盘共物体基线 | 两臂在大箱顶部两个吸附点同时抓取 | ✅ 双 Surface Gripper 同时 CLOSED；未抬升阶段 SharedBox 三维位移 0.115 mm |
| 12 | 紧协调同步搬运 | ✅ 共同抬升与 +100 mm X 向共同运输均通过 Isaac 验收；运输误差 0.311 mm、相对 link8 误差 0.123 mm |
| 13 | 紧协调放置与释放 | ✅ 共同下降、同步释放、Ground Truth 回写与预规划安全退出均已通过 Isaac 验收；放置误差 0.947 mm |
| 14 | 紧协调改进控制 | 🟡 Task14-A 已实现连续相对 TCP / SharedBox 几何观测，待 Isaac 验收；据此决定是否需要反馈、柔顺或负载分配 |
| 15 | 紧 / 松协调混合码垛 | 先复用 Task11--13 紧协调搬运一个大件，再复用 Task07--10 松协调以两批次并行堆叠四个小件 | 🟡 场景与项目级复用 skill 已就绪，待专用 Bridge / 控制器与 Isaac 验收 |
