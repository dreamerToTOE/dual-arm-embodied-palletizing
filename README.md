...
| 26 | 车厢三面围墙 + 预推入位（第一层 4 件） | 🟡 布局已按用户重设计并提交（**X 向导轨 + 车厢 Y 居中 +X**，**Isaac 掉线待实测**）：导轨沿 ±X（两臂各一条，行程 [0.25,0.70]、静止位 0.45、基座 Y 固定 ±0.60）；车厢内腔 x∈[0.66,0.91]、y∈[-0.125,0.125]（Y 居中），开口 −X、推入 +X；格位 (0.84/0.72, ±0.065)，预推位 x=0.54；桌面加宽 1.5 m、供料槽西移 (0.28, ±0.10)。X 向导轨让推入方向与导轨轴一致、且整条导轨在桌面外侧（不再有肩部压桌面的内向行程上限）。执行器仍是最初 −Y 布局，待重做 |
