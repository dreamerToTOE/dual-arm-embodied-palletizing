...
- [Task 27 — 五 Cube 第一层：边侧基准 + 中心插入](docs/tasks/TASK27_FIVE_CUBE_CENTER_INSERT.md)
...
| 26 | 车厢三面围墙 + 预推入位（两件 `+Y` 行基线） | ✅ 已完成两件 Isaac 物理闭环，并完成独立重复复测。深位/浅位 Cube 均完成紧协调搬运、`+X` 推入、`+Y` 侧压贴墙、安全退出与共同 HOME；最新复测的最差位置误差 `1.101 mm`，最差支撑/侧墙间隙 `1.088 mm`。重抓阶段采用三批 RRTConnect 候选并以“重抓→推入→退出”整链 FCL 筛选，避免随机腕部冗余分支造成后续 Cartesian 推入失效。 |
| 27 | 五 Cube 第一层：边侧基准 + 中心插入 | 🟡 已实现独立 Task27 场景、`/task27` bridge 和执行器，并通过 Python 语法检查与 `fr3_dual_palletize` 构建。阶段 A 按 `+2 → -2 → +1 → -1` 左右交替复用 Task26 的贴墙/贴 Cube 工艺；阶段 B 可用独立进程恢复前四件 Ground Truth CollisionObject 后只执行中心件。当前 `0.628 m` 内腔容纳五件 `0.120 m` Cube 后必有 `28 mm` 总余量，中心件目标是对称分配约 `14 mm + 14 mm`；待 Isaac / MoveIt 零命令预检和物理验收。 |
