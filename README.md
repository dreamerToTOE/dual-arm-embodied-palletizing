...
| 27 | 五 Cube 第一层：边侧基准 + 中心插入 | 🟡 方案已冻结，待实施。阶段 A 按 `+2 → -2 → +1 → -1` 左右交替复用 Task26 的贴墙/贴 Cube 工艺；阶段 B 单独研究中心 Cube 插入。当前 `0.628 m` 内腔容纳五件 `0.120 m` Cube 后必有 `28 mm` 总余量，中心件目标是对称分配约 `14 mm + 14 mm`，而非不物理的双侧零缝。 |
