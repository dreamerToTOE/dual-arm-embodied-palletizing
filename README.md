...
| 27 | 五 Cube 第一层：边侧基准 + 中心插入 | ✅ 2026-09-24 完成一次 Isaac Sim 4.5 + MoveIt2 五件实体码垛：阶段 A 四件、阶段 B 中央件均 exit 0，双臂回 HOME。外侧件贴墙、内侧件按外侧实测位置对齐通道后直推；中央件误差 `0.483 mm`，两侧余隙 `12.314 / 14.151 mm`。修复分阶段续跑时滑轨 `0.100 m` 偏移未恢复的问题。仍需复现实验统计稳定性及复核一次 `63.87 N·m` 短时峰值；详见 [Task27 记录](docs/tasks/TASK27_FIVE_CUBE_CENTER_INSERT.md)。 |
