...
| 22 | Ground Truth 在线任务分派与预规划 | 🟡 Task22-A typed dispatch 与 Task22-B 真实 MoveIt/FCL tight 预检已通过（856 FCL samples，未执行）；Task22-C1 预测缓存的 GT/terminal-state 复用门禁已回归通过；Task22-C2 独立 planning sandbox 已验证 scene 隔离与实时 joint-state 接入，sandbox 候选生成待接入 |
