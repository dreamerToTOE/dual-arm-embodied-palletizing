...
| 27 | 五 Cube 第一层：边侧基准 + 中心插入 | ✅ 2026-09-26 当前 `1.5 mm` 内侧余量版本在 Isaac Sim 4.5 + MoveIt2 中从初始场景**单进程连续完成五件**，exit 0、双臂回 HOME；五件格位误差最大 `1.606 mm`，中央件误差 `0.651 mm`，两侧余隙 `10.636 / 11.886 mm`，分段推入记录峰值最大 `34.71 N·m`。旧版 2026-09-24 的分阶段成功记录保留；两版几何不同，当前版仍需多轮同配置重复统计。详见 [Task27 记录](docs/tasks/TASK27_FIVE_CUBE_CENTER_INSERT.md)。 |
