...
| 26 | 车厢三面围墙 + 预推入位（第一层 4 件） | 🟡 **场景与 bridge 已按最终布局重做并实测通过**：车厢三面墙（+X / +Y / −Y，厚 20 mm、高 250 mm），**开口在 −X 面**（用户指定），内腔 250×250（每侧 5 mm 净空）；格位 (0.845/0.725, −0.065/−0.185)，预推位 x=0.545，推入方向 **+X**。两个机械臂**各一条 Y 向导轨**（静止位 ±0.600 = Task24/25 基座；行程**不对称**：左 [−0.72,−0.48]、右 [+0.55,+0.72]，受肩部外壳与桌面内沿约束，向内净空 25/20.6 mm；滑台顶面 z=0 故不改基座高度），由 bridge 的 rail_command/rail_state 驱动（USD+Fabric 同值双写，吸盘夹紧或 1 s 内有关节命令时拒绝动轨）；实测两臂平移到位、三处读数一致、批 1 到料正常。**执行器仍是旧 −Y 布局，推入方向 Y→X 的重做 + 推入姿态 FCL 预检是下一步** |
